...
class Ctr__generateur extends Ctr_controleur{
    ...
    function a_magicAllTables() {
        ...
        $sql = "show tables";
        $result = Entity::$link->query($sql,PDO::FETCH_BOTH);
    ...
    private function creerClassTable($nomTable) {
        ...
        file_put_contents("../_entity/$nomClasse.class.php", $chaine);
    }
    ...
    private function genererFormulaire($nomTable) {
        $chaine = "";
        $nomCle = substr($nomTable, 0, self::$lprefixe) . "_id";
        $result = Entity::$link->query("SHOW COLUMNS FROM $nomTable",PDO::FETCH_BOTH);
        foreach ( $result as $row) {
            /*
            echo "<pre>";
            print_r($row);
