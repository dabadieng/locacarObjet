...
alter table vehicule add constraint cs3 foreign key (veh_site) references agence(age_id);
alter table vehicule add constraint cs4 foreign key (veh_categorie) references categorie(cat_id);
...
alter table locations add constraint cs7 foreign key (loc_client) references utilisateur(uti_id);
alter table locations add constraint cs8 foreign key (loc_gestionnaire) references utilisateur(uti_id);
alter table locations add constraint cs9 foreign key (loc_vehicule) references vehicule(veh_id);
alter table utilisateur add constraint cs11 foreign key (uti_agence) references agence(age_id);
...
alter table utilisateur add constraint cs16 foreign key (uti_profil) references profil(pro_id);

set foreign_key_checks =1;

-- vue donnant le prix d’une location (hors options sur le véhicule).
...
-- vue pour chiffre d’affaire d’une agence donnée.
